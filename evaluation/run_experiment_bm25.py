# evaluation/run_bm25_on_cache.py
...
def load_corpus_from_cache(method_name: str, params: dict) -> pd.DataFrame:
    # enforce cache-only (no rebuilding)
    def _builder():
        raise RuntimeError(
...
def main():
    ...
    RANDOM_STATE = 42
    TEST_SIZE = 0.5
    MAX_QUERIES = 200        # ✅ quick method selection; set None for full
    MIN_PLACES = 50

    HEARTBEAT_EVERY_EVAL = 25     # print every N evaluated queries
    HEARTBEAT_EVERY_TRY  = 50     # print every N attempted queries (even if skipped)
    ...
    print("\nRunning BM25 evaluation on cached corpora...")
    for method_name, module_path, params in iter_methods():
        print("\n===================================")
        ...
        candidate_set = set(retriever.place_ids)

        id_to_text = dict(zip(df_train["place_id"], df_train["text"]))
        query_ids = list(id_to_text.keys())

        # shuffle queries so the first 200 are representative
        ...
        if n_used == 0:
            print("Skipping: no valid queries.")
            continue
        ...
        results.append(row)
        print("Done.", method_name, "| nDCG@10:", row.get("nDCG@10"), "| mrr_l2:", row.get("mrr_l2"))
    ...
    results_df.to_csv("results/bm25_selection_methods_full_metrics.csv", index=False)
    results_df.head(5).to_csv("results/bm25_top5_selection_methods.csv", index=False)

    print("\nSaved results:")
    print(" - results/bm25_selection_methods_full_metrics.csv")
    print(" - results/bm25_top5_selection_methods.csv")
